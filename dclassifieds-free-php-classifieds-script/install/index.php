...
if(!version_compare(PHP_VERSION,"5.1.0",">=")){
	$error_array[] = 'PHP Version Failed. Must be >= 5.1.0';
}

if(!class_exists('Reflection',false)){
	$error_array[] = 'Reflection class is not loaded.';
}

if(!extension_loaded("pcre")){
	$error_array[] = 'PCRE Extension is not loaded.';
}

if(!extension_loaded("SPL")){
	$error_array[] = 'SPL Extension is not loaded.';
}

if(!extension_loaded('pdo')){
	echo 'Failed';
	$error_array[] = 'PDO Extension is not loaded.';
}

if(!extension_loaded('pdo_mysql')){
	echo 'Failed';
	$error_array[] = 'PDO_MySQL Extension is not loaded.';
}

if(!extension_loaded("mcrypt")){
	echo 'Failed';
	$error_array[] = 'MCrypt Extension is not loaded.';
}
...
if(!is_writable($assets_path)){
	$error_array[] = 'Assets path (' . $assets_path . ') is not writable.';
}

if(!is_writable($config_path)){
	$error_array[] = 'Config path (' . $config_path . ') is not writable.';
}

if(!is_writable($uf_path)){
	$error_array[] = 'User Files path (' . $uf_path . ') is not writable.';
}

if(!is_writable($runtime_path)){
	$error_array[] = 'Cache and Log path (' . $runtime_path . ') is not writable.';
}

if(!empty($error_array)){
	echo 'DClassifieds Free PHP Classifieds Script Installation<br />';
	echo 'Please correct this errors:<br />';
	$i =1;
