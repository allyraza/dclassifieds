...
Class Asido_Driver_GD Extends Asido_Driver {
	...
	*/
	function __canvas($width, $height, &$color) {
		...
		$generated_color = imagecolorallocate($t->target, $r, $g, $b);//dinko
		
		imagefill($t->target, 1, 1, $generated_color); //dinko
			
		imagefilledrectangle($t->target, 0, 0, $width - 1, $height - 1, $generated_color);	//dinko
